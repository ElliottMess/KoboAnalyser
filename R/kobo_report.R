...
#' @description  Automatically generate the report.
#'
...
kobo_report <- function() {
  ...
  for( i in 1:nrow(chapters) )
  {
    ...
    cat(paste("title: \" ", report_name, " -  ", chaptersname , "  \"", sep=""), file = chapter.name ,sep="\n", append=TRUE)
    cat(paste("author: \"Analysis by: ", author, " \"", sep=""), file = chapter.name ,sep="\n", append=TRUE)
    cat(paste("date: \"",location," , on  `r format(Sys.Date(),  '%d %B %Y')`\"", sep=""),file = chapter.name ,sep="\n", append=TRUE)
